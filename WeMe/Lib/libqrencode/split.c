...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	while(isalnum(*p)) {
		if(isdigit(*p)) {
            ...
            int PInt = (int)(p - string);
            int QInt = (int)(q - string);
            int QP = (int)(q - p);
			dif = QRinput_estimateBitsModeAn(PInt) + QRinput_estimateBitsModeNum(QP) + 4 + ln - QRinput_estimateBitsModeAn(QInt);
			if(dif < 0) {
				break;
			} else {
				p = q;
...
static int Split_eat8(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	while(*p != '\0') {
		...
		if(mode == QR_MODE_NUM) {
            ...
            int QInt = (int)(q - string);
            int QP = (int)(q - p);
			dif = QRinput_estimateBitsMode8(PInt) /* + 4 + l8 */
				+ QRinput_estimateBitsModeNum(QP) + 4 + ln
				- QRinput_estimateBitsMode8(QInt) /* - 4 - l8 */;
			if(dif < 0) {
				break;
			} else {
				p = q;
            ...
            int QP = (int)(q - p);
			dif = QRinput_estimateBitsMode8(PInt) /* + 4 + l8 */
				+ QRinput_estimateBitsModeAn(QP) + 4 + la
				- QRinput_estimateBitsMode8(QInt) /* - 4 - l8 */;
			if(dif < 0) {
				break;
			} else {
