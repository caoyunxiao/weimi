...
#define kWemeDBName @"weme.sqlite"
...
- (id) init {
    ...
    if (self)
    {
        int state = [self initializeDBWithName:kWemeDBName];
        
        [self deleteDB:kDefaultDBName];
...
- (void)deleteDB:(NSString *)name
{
    ...
    // 沙盒Docu目录
    NSString * docp = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES) objectAtIndex:0];
    NSString *oldName = [docp stringByAppendingString:[NSString stringWithFormat:@"/%@",name]];
    NSFileManager * fileManager = [NSFileManager defaultManager];
    BOOL exist = [fileManager fileExistsAtPath:oldName];
    
    if (exist)
    {
        [fileManager removeItemAtPath:oldName error:nil];
    }
}
